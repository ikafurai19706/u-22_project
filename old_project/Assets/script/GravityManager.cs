...
public class GravityManager : MonoBehaviour
{
    ...
    void Update()
    {
		...
		if((Input.GetKey(KeyCode.RightShift) || Input.GetKey(KeyCode.LeftShift)) && stay)
		{
			...
			{
				nowgravity = max(
								new Vector3(PlayerController.forward.x,0,0),
								new Vector3(0,PlayerController.forward.y,0),
								new Vector3(0,0,PlayerController.forward.z)
								).normalized * 98.1f;
			}
			else if(Input.GetKey(KeyCode.S))
			{
				nowgravity = max(
								new Vector3(PlayerController.back.x,0,0),
								new Vector3(0,PlayerController.back.y,0),
								new Vector3(0,0,PlayerController.back.z)
								).normalized * 98.1f;
			}
			else if(Input.GetKey(KeyCode.D))
			{
				nowgravity = max(
								new Vector3(PlayerController.right.x,0,0),
								new Vector3(0,PlayerController.right.y,0),
								new Vector3(0,0,PlayerController.right.z)
								).normalized * 98.1f;
			}
			else if(Input.GetKey(KeyCode.A))
			{
				nowgravity = max(
								new Vector3(PlayerController.left.x,0,0),
								new Vector3(0,PlayerController.left.y,0),
								new Vector3(0,0,PlayerController.left.z)
								).normalized * 98.1f;
			}
		}
